...
class HTTPClient(): # For HTTP/1.X
    ...
    def get(self, url, headers=None, stream=False):
        ...
        try:
            response = Response(self.socket, stream)#, recv_bytes.decode())
            # print('get server response:', response)
        except:
            self.socket.close()
            print("socket closed 0!")
            return None
        return response
